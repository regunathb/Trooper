...
package org.trpr.platform.servicefw.jetty;
...
public class JettyWebAppContextFactory  implements FactoryBean<WebAppContext> {
	...
	public WebAppContext getObject() throws Exception {
		...
		for (File file : files) {
			...
			if (fileToString.contains(".jar!") && fileToString.startsWith("file:"+File.separator)) {
				fileToString = fileToString.replace("file:"+File.separator,"jar:file:"+File.separator);
